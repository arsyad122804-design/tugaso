...
const MosqueSilhouette = () => (
  <div className="absolute bottom-0 left-0 w-full overflow-hidden pointer-events-none">
    <svg viewBox="0 0 800 250" className="w-full h-64">
      ...
      {/* Cahaya lampu dari jendela masjid */}
      <ellipse cx="400" cy="160" rx="25" ry="35" fill="url(#lampGlow)" className="animate-lamp-flicker" />
      <ellipse cx="300" cy="180" rx="20" ry="30" fill="url(#lampGlow)" className="animate-lamp-flicker" style={{ animationDelay: '0.5s' }} />
      <ellipse cx="500" cy="180" rx="20" ry="30" fill="url(#lampGlow)" className="animate-lamp-flicker" style={{ animationDelay: '1s' }} />
      
      {/* Kubah tengah besar */}
      <ellipse cx="400" cy="130" rx="70" ry="50" fill="#0f172a" />
      <rect x="360" y="130" width="80" height="120" fill="#0f172a" />
      
      {/* Jendela kubah tengah - bercahaya */}
      ...
      {/* Bulan sabit di puncak kubah tengah */}
...
// Komponen Lampu Gantung (seperti lampu masjid)
...
export default function LoginPage() {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Update CSS variables for custom cursor
      document.documentElement.style.setProperty('--mouse-x', `${e.clientX}px`)
      document.documentElement.style.setProperty('--mouse-y', `${e.clientY}px`)
      
      // Create particle on mouse move - warna tema Ramadhan
      const colors = ['#FEF08A', '#E0F2FE', '#BAE6FD', '#7DD3FC', '#38BDF8', '#0EA5E9']
      const newParticle = {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-indigo-950 via-blue-950 to-slate-900 px-4 relative overflow-hidden">
      ...
      <div 
        className="pointer-events-none fixed w-40 h-40 rounded-full blur-2xl opacity-20 transition-all duration-500 ease-out z-0"
        style={{
          left: mousePosition.x - 80,
          top: mousePosition.y - 80,
          background: 'radial-gradient(circle, #FEF08A, #7DD3FC, transparent)'
        }}
      />
      ...
      {particles.map((particle) => (
        <div
          key={particle.id}
          className="pointer-events-none fixed animate-particle-fade z-0"
          style={{
            left: particle.x,
            top: particle.y,
            width: particle.size,
